...
class Controller extends BaseController
{
        ...
        if (($Number ) >= 999) {
                    ...
                    if (!$decimal) {
                        if ($type == 1) {
                            ...
                            if (date('m') == 01 || date('m') == 02 || date('m') == 03)
                                return date('y', strtotime('-1 year')) . date('y') . 'ID-' . "9999";
                            else
                                return date('y') . date('y', strtotime('+1 year')) . 'ID-' . "9999";
                        }
                    }
                    if ($type == 1) {
                        if (date('m') == 01 || date('m') == 02 || date('m') == 03)
                        ...
                        if (date('m') == 01 || date('m') == 02 || date('m') == 03)
                            return  date('y', strtotime('-1 year')) . date('y') . 'ID-' . substr($decimal, 0, 4);
                        else
                            return date('y') . date('y', strtotime('+1 year')) . 'ID-' . substr($decimal, 0,);
                    }
                } else {
                    $Number = (($Number + 1) / 999);
                    list($whole, $decimal) = explode('.', $Number);
    ...
    function year_combo_num_gen($Number)
    {
        ...
        if ($Number == 0) {
            // Handling special case when $Number is 0
            if ($currentMonth <= "03") {
                // If the current month is January or February, use the previous year
                return date('y', strtotime('-1 year')) . date('y') . '-0001';
            } else {
                // Otherwise, use the current year
                return date('y') . date('y', strtotime('+1 year')) . '-0001';
            }
        } else {
            // Construct the desired result format for other cases
            if ($Number == 998) {
                if ($currentMonth <= "03") {
                    return date('y', strtotime('-1 year')) . date('y') . '-999';
                } else {
                ...
                if ($currentMonth <= "03") {
                    return date('y', strtotime('-1 year')) . date('y') . '-' . sprintf('%04d', $decimal);
                } else {
                    return date('y') . date('y', strtotime('+1 year')) . '-' . sprintf('%04d', $decimal);
                }
            } else {
                // If $Number is less than 999, calculate the decimal part
                $decimal = ($Number + 1) % 999;
                if ($decimal == 0) {
                    $decimal = 999;
                }
                // Format the decimal part with leading zeros
                $formattedDecimal = sprintf('%04d', $decimal);

                // Determine the year based on the current month
                if ($currentMonth <= "03") {
                    $baseYear = date('y', strtotime('-1 year')) . date('y');
                } else {
                    $baseYear = date('y') . date('y', strtotime('+1 year'));
                }

                // Construct the result
                return  $baseYear . '-' . $formattedDecimal;
            }
        }
    }
