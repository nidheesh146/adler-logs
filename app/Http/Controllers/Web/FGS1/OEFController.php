...
class OEFController extends Controller
{
       ...
       // dd('test');
        if ($request->isMethod('post')) {
            ...
            if (!$validator->errors()->all()) {
                ...
                // Generate latest oef_number
                $prefix = 'OEF-' . $years_combo . '-';
                $latest = DB::table('fgs_oef')
    ...
    public function edit_oef($id)
    {
        $oef = fgs_oef::select(
            ...
            $transaction_type = transaction_type::get();
            $order_fulfil = order_fulfil::get();
            return view('pages/FGS/OEF/OEF-edit', compact('oef','category','transaction_type','order_fulfil'));
    }
    ...
    public function OEFproductsearch(Request $request, $oef_id)
    {
        ...
        foreach ($data as $dat) {
            if ($dat['gst'] != '') {
                if($customer['zone_name']=='Export')
                {
                    $gst_data = inventory_gst::select('inventory_gst.*')->where('inventory_gst.igst', '=', 0)->where('inventory_gst.sgst', '=', 0)->where('inventory_gst.cgst', '=', 0)->first();
                }
                else
                {
    ...
    public function OrderAcknowledgementEmail($oef_id,Request $request)
    {
        ...
        //$mailData->to = ['chandrashekhar.purohit@adler-healthcare.com'];
        $supp = DB::table('customer_supplier')
                            ...
                            ->where(['id'=>$oef->customer_id])->first();
        //$receipter[] = 'chandrashekhar.purohit@adler-healthcare.com';  
        //$receipter1[] = 'shilma33@gmail.com';  
        if($supp->email!=NULL)
