...
class DNIController extends Controller
{
    ...
    public function DNIAdd(Request $request)
    {
        if($request->isMethod('post'))
        {
            ...
            if(!$validator->errors()->all())
            {
                ...
                // $data['dni_number'] = "DNI-" . $years_combo . "-" . str_pad(
                //     DB::table('fgs_dni')
    ...
    {
        $dni_item = fgs_dni_item::where('id',$dni_item_id)->first();
        $maa_stock =  fgs_maa_stock_management::where('product_id',$dni_item['product_id'])->where('batchcard_id',$dni_item['batchcard_id'])->first();
        if($maa_stock)
        {
            $stock['quantity'] =$maa_stock['quantity']+$dni_item['remaining_qty_after_srn'];
            $maa_stock_management = $this->fgs_maa_stock_management->update_data(['id'=>$maa_stock['id']],$stock);
        }
        else
        {
            $stock['product_id'] = $dni_item['product_id'];
            $stock['batchcard_id'] = $dni_item['batchcard_id'];
            $stock['quantity'] = $dni_item['remaining_qty_after_srn'];
            $stock['created_at'] = date('Y-m-d H:i:s');
            $stock_add = $this->fgs_maa_stock_management->insert_data($stock);
        }
        DB::table('fgs_dni_item')->where('fgs_dni_item.id','=',$dni_item_id)
                    ->delete();
        DB::table('fgs_dni_item_rel')
                    ->where('fgs_dni_item_rel.item','=',$dni_item_id)
                    ->delete();
        session()->flash('success', "You have  successfully deleted DNI Item !");
        return redirect()->back();
    }
    ...
    public function DNIitemlist($dni_id)
    {
        ...
        'product_product.sku_code','product_product.hsn_code','product_product.discription','fgs_grs.grs_number','fgs_oef_item.rate','fgs_oef_item.discount',
        'inventory_gst.igst','inventory_gst.cgst','inventory_gst.sgst','inventory_gst.id as gst_id','fgs_dni_item.quantity','currency_exchange_rate.currency_code','fgs_pi.pi_number')
                            ->leftJoin('fgs_dni_item','fgs_dni_item.id','fgs_dni_item_rel.item')
                            ->leftJoin('fgs_pi','fgs_pi.id','=','fgs_dni_item.pi_id')
                            ->leftjoin('product_product','product_product.id','=','fgs_dni_item.product_id')
    ...
    public function dni_transaction(Request $request)
    {
        ...
        $items=fgs_dni_item::select('fgs_dni.*','product_product.sku_code','product_product.discription','product_product.hsn_code','fgs_pi_item.batch_qty as quantity',
