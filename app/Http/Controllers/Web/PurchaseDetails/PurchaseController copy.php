...
class PurchaseController extends Controller
{
    ...
    public function getRQNumber($id)
    {
        
        // Retrieve values from the table based on $id
        $value = DB::table('inv_purchase_req_quotation')->select('rq_no')->where('quotation_id', '=', $id)->first();

        $data = $value->rq_no;
        return ($data);
    }

    public function getQuotationDate($id)
    {
        
        // Retrieve values from the table based on $id
        $value = DB::table('inv_purchase_req_quotation_supplier')->select('quotation_date','id as spid','commited_delivery_date')->where('quotation_id', '=', $id)->first();
    ...
    public function insertFinalPurchase(Request $request)
    {
        ...
        if (!$validator->errors()->all()) {
            foreach ($request->quotation_id as $quotation_id) {
                $groupByItemSupplier = $this->inv_purchase_req_quotation_item_supp_rel
                    ->groupByItemSupplier(['inv_purchase_req_quotation_item_supp_rel.selected_item' => 1, 'inv_purchase_req_quotation_item_supp_rel.quotation_id' => $quotation_id]);
                ...
                foreach ($groupByItemSupplier as $ByItemSupplier) {
                    ...
                    if ($type == "PR") {
                        ...
                        if ($item_type2 == "Finished Goods") {
                            ...
                            $po_number = "POI1-" . $this->po_num_gen(DB::table('inv_final_purchase_order_master')->where('po_number', 'like', '%POI1-' . $years_combo . '%')
                                ->Orwhere('po_number', 'like', '%POC1-' . $years_combo . '%')->where('type', '=', 'PO')->count(), 1);
                            $pono = inv_final_purchase_order_master::where('po_number', $po_number)->first();
                           
                            if ($pono) {
                                $data['po_number'] = "POI1-" . $this->po_num_gen(DB::table('inv_final_purchase_order_master')->where('po_number', 'like', '%POI1-' . $years_combo . '%')
                                    ->Orwhere('po_number', 'like', '%POC1-' . $years_combo . '%')->where('type', '=', 'PO')->count() + 1, 1);
                            } else {
                                $data['po_number'] = "POI1-" . $this->po_num_gen(DB::table('inv_final_purchase_order_master')->where('po_number', 'like', '%POI1-' . $years_combo . '%')
                                    ->Orwhere('po_number', 'like', '%POC1-' . $years_combo . '%')->where('type', '=', 'PO')->count(), 1);
                            }
                            ...
                            if ($item_type == "Direct Items") {
                                ...
                                $po_number = "POI2-" . $this->po_num_gen(DB::table('inv_final_purchase_order_master')->where('po_number', 'like', '%POI2-' . $years_combo . '%')
                                    ->Orwhere('po_number', 'like', '%POC2-' . $years_combo . '%')->where('type', '=', 'PO')->count(), 1);
                                $pono = inv_final_purchase_order_master::where('po_number', $po_number)->first();
                                ...
                                $po_number = "POI3-" . $this->po_num_gen(DB::table('inv_final_purchase_order_master')->where('po_number', 'like', '%POI3-' . $years_combo . 'ID%')
                                ->Orwhere('po_number', 'like', '%POC3-' . $years_combo . 'ID%')->where('type', '=', 'PO')->count() + 2, 1);
                            $pono = inv_final_purchase_order_master::where('po_number', $po_number)->first();
                    ...
                    $previous = DB::table('inv_final_purchase_order_master')->select('*')->orderby('id', 'desc')->first();
                    $quotation_data = DB::table('inv_purchase_req_quotation')
                    ->select('*')
                    ->where('quotation_id',$data['rq_master_id'])
                    ->first();
                   
                    $current_data = DB::table('inv_purchase_req_quotation_supplier')
                    ->select('inv_purchase_req_quotation_supplier.supplier_id','inv_purchase_req_quotation_supplier.quotation_id','inv_purchase_req_quotation.date','inv_purchase_req_quotation.delivery_schedule')
                    ->where('supplier_id',$previous->supplier_id)
                    ->leftJoin('inv_purchase_req_quotation', function($leftJoin)
                        {
                          $leftJoin->on('inv_purchase_req_quotation.quotation_id', '=', 'inv_purchase_req_quotation_supplier.quotation_id');

                        })
                    ->where('inv_purchase_req_quotation.date',$quotation_data->date)
                    ->where('inv_purchase_req_quotation.delivery_schedule',$quotation_data->delivery_schedule)
                    ->get();

                   //dd($quotation_data,$current_data, $request);
                    if(count($current_data) != 0){
                       $po_number = $previous->po_number;
                       $data['po_number'] =  $po_number;
                       foreach($request->quotation_id as $quotation_ids)
                         {
                           $rq_master_id = $quotation_ids;
                         }
                       // Find the latest POMaster instance based on the previous id
                        $POMaster = inv_final_purchase_order_master::find($previous->id);

                       // Set rq_master_id to null to delete existing data
                        // $POMaster->rq_master_id = null;
                        // $POMaster->save();

                        // Combine the array of quotation_id values into a comma-separated string
                       $rq_master_id = implode(',', $request->quotation_id);

                      // Assign the new rq_master_id value to the model
                       $POMaster->rq_master_id = $rq_master_id;

                      // Save the model to apply the new value
                      $POMaster->save();

                      $data['rq_master_id'] = $rq_master_id;

                      // Save the model to apply the new value
                      $POMaster = $this->inv_final_purchase_order_master->insert_data1($previous->id,$data, $inv_supplier_terms->terms_and_conditions);
        ...
        else
        {
            if ($request->isMethod('post')) {
                ...
                if (!$validator->errors()->all()) {
                    ...
                    $po_item = inv_final_purchase_order_item::where('id', $id)->first();
                    $POitem = $this->inv_final_purchase_order_item->updatedata(['id' => $id], $data);
                    $po_master =inv_final_purchase_order_rel::where('item','=', $id)->pluck('master')->first();
                    
                    if (date('Y-m-d',strtotime($request->delivery_schedule))!=$po_item['delivery_schedule'] && $request->quantity==$po_item['order_qty'] && $request->rate==$po_item['rate'] && $request->discount==$po_item['discount'] && $request->Specification==$po_item['Specification'] ) {
...
/*Supplier Invoice */
    public function supplierInvoiceAdd(Request $request)
    {
        if ($request->isMethod('post'))
        {
            ...
            if(!$validator->errors()->all())
            {
                ...
                $data['material_receipt_date'] =date('Y-m-d', strtotime($request->reciept_date));
    ...
    public function generateFinalPurchasePdf($id, $order=null, Request $request)
    {
        ...
        // $data['rq_number'] = $this->getRQNumber($data['final_purchase']->rq_master_id);
        // dd($id);
    ...
    {
        $condition2=[];
        $condition1[] = ['inv_final_purchase_order_master.po_date', '>=', date('Y-m-d', strtotime('03-07-2024'))];
        if ($request->supplier) {
    ...
    public function R02pendingPurchaseRealisation(Request $request)
    {
        ...
        $data = inv_final_purchase_order_master::select([
            ...
            'inv_final_purchase_order_master.po_number',
             'inv_final_purchase_order_master.status', 
            'inv_final_purchase_order_master.id as po_id',
             'inv_final_purchase_order_master.created_at',
            'user.f_name', 'user.l_name',
             'inv_final_purchase_order_master.id',
            ...
            'inv_purchase_req_master.pr_no',
            'inv_purchase_req_item_approve.updated_at as pr_date',
            'inv_purchase_req_item_approve.status as pr_status',
            ...
            ->leftjoin('inventory_gst', 'inventory_gst.id', '=', 'inv_final_purchase_order_item.gst')
            ->leftjoin('inv_item_type', 'inv_item_type.id', '=', 'inventory_rawmaterial.item_type_id')
            ->leftjoin('inv_purchase_req_item_approve', 'inv_purchase_req_item.requisition_item_id', '=', 'inv_purchase_req_item_approve.pr_item_id')
            ->orderby('inv_final_purchase_order_master.id', 'asc')
            // ->where('inv_final_purchase_order_rel.master', '=', $po['id'])
            ->where('inv_final_purchase_order_item.qty_to_invoice', '!=', 0)
            ->where('inv_final_purchase_order_item.cancelled_qty', '=', 0)
            ->where($condition2)
            
            ->whereNotExists(function ($query) {
                $query->select(DB::raw(1))
                      ->from('inv_supplier_invoice_item')
                      ->whereRaw('inv_supplier_invoice_item.po_item_id = inv_final_purchase_order_item.id');
            })
            ->orderBy('inv_final_purchase_order_item.id', 'ASC')
            ->get();
            // dd($data);
        return Excel::download(new R02pendingpurchaseExport($data), 'R02-PendingPurchaseRealisation1' . date('d-m-Y') . '.xlsx');
    }
