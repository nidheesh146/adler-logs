...
class PIPayment extends Mailable
{
    ...
    public function build()
    {
        if (!empty($this->mailData)) {
            $data['pi'] = DB::table('fgs_pi')
                ->select('fgs_pi.*', 'fgs_oef.oef_number', 'fgs_oef.oef_date', 'order_fulfil.order_fulfil_type', 'fgs_oef.order_number', 'fgs_oef.order_date', 'fgs_grs.grs_number', 'fgs_grs.grs_date',
                ...
                ->where('fgs_pi.id', '=', $this->mailData->pi_id)
                ->where('fgs_grs.status', '=', 1)
                ->first();
            $pi_number = $data['pi']->pi_number;

            $data['items'] = DB::table('fgs_pi_item_rel')
                ->select('fgs_grs.grs_number', 'fgs_grs.grs_date', 'fgs_item_master.sku_code', 'fgs_oef.oef_number', 'fgs_oef.oef_date', 'fgs_oef.order_number', 'fgs_oef.order_date', 'fgs_item_master.hsn_code', 'fgs_item_master.discription',
                ...
                ->where('fgs_pi_item_rel.master', '=', $this->mailData->pi_id)
            ...
            if ($supp->asm_email != NULL) $cc[] = $supp->asm_email;
            if ($supp->zm_email != NULL) $cc[] = $supp->zm_email;
            if ($supp->rm_email != NULL) $cc[] = $supp->rm_email;
            if ($supp->me_email != NULL) $cc[] = $supp->me_email;

            return $this->view('emails.pi_payment', compact('mailData'))
                ->cc($cc)
                ...
                ->attachData($pdf->output(), $pi_number . ".pdf");
        }
    }
}
