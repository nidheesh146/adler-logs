                                        ...
                                        <div class="row filter_search" style="margin-left: 0px;">
                                            ...
                                            <div class="col-sm-10 col-md-10 col-lg-10 col-xl-10 row">
                                                ...
                                                <div class="form-group col-sm-12 col-md-2 col-lg-2 col-xl-2">
                                                    ...
                                                    <input type="text" name="subscriber_id"  value="{{request()->get('subscriber_id')}}" class="form-control" placeholder="Subscriber ID">
                                                </div><!-- form-group -->
                            ...
                            @foreach ($data['suscriber'] as $suscriber)
                                <tr>
                                    <th>
                                        ...
                                        <a href=" {{ url('subscribers/create/' . $type[$suscriber['type_id']].'/'.$Controller->hashEncode($suscriber['subr_id'])) }}" >{{$suscriber['subscriber_id']}}</a>
                                        @else
                                    ...
                                    <td>
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <a href="{{ url('subscription/create/'.$type[$suscriber['type_id']].'/'.$Controller->hashEncode($suscriber['subr_id']))  }}"
                                                class="dropdown-item"><i class="fas fa-money-check-alt" style="font-size: 13px;margin: 0;"></i> Subscription
                                            </a>
                                            @endif
                                            @if (in_array('subscribers.edit', config('permission')))  
                                                <a href=" {{ url('subscribers/create/' . $type[$suscriber['type_id']].'/'.$Controller->hashEncode($suscriber['subr_id'])) }}" class="dropdown-item"><i class="fas fa-edit"></i> Edit</a>
                                            @endif          
                                            @if (in_array('subscribers.delete', config('permission')))  
                                             <a href=" {{ url('subscribers-action/' . $org.'/'.$Controller->hashEncode($suscriber['subr_id'])).'/action' }}"  class="dropdown-item" onclick="return confirm('Are you sure you want to {{($suscriber['status'] == 1) ? 'deactive' : 'active' }} this ?');" ><?=($suscriber['status'] == 1) ? '<i class="fas fa-times"></i> Deactive' : '<i class="fas fa-check"></i> Active' ;?></a>
                                             <a href=" {{ url('subscribers-action/' . $org.'/'.$Controller->hashEncode($suscriber['subr_id'])).'/delete' }}" onclick="return confirm('Are you sure you want to delete this ?');"  class="dropdown-item"><i class="fas fa-trash-alt"></i>  Delete</a>
                                            @endif   
                                        </div>
