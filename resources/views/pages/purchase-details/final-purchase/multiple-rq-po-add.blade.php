		...
		<div class="az-content-body">
			<div class="az-content-breadcrumb"> 
				<span><a href="">Order</a></span>
				 <span><a href=""> Multiple RQ @if(request()->get('order_type')=='wo') Work @else Purchase @endif Order</a></span>
				 </div>
			<h4 class="az-content-title" style="font-size: 20px;"> Multiple RQ @if(request()->get('order_type')=='wo') Work @else Purchase @endif Order
		   ...
		   @endif
			...
			<div class="row row-sm mg-b-20 mg-lg-b-0">
				...
				<div class="tab-pane tab-pane active  show" id="purchase">
                    ...
                    <form method="post" action="{{url('inventory/multiple-RQ-purchase-insert')}}">
                    {{ csrf_field() }}
                    <input type="hidden" value="{{request()->get('order_type')}}" id="order_type"  name="order_type">
					<div class="table-responsive">
						<table class="table table-bordered mg-b-0" id="example1">
                            ...
                            @foreach($data['quotation'] as $item)
							    <tr>
									<td><input type="checkbox" class="quotation_id" id="quotation_id" name="quotation_id[]" value="{{$item['quotation_id']}}" onClick="enable(this)"></td> 
									<td>{{$item['rq_no']}} - {{$SupplierQuotation->getItemCount($item['quotation_id'])}}</td>
									<td>{{$item['date'] ? date('d-m-Y',strtotime($item['date'])) : '-'}}</td>
									<td><input type="text" name="delivery_dates[]" value="{{$item['delivery_schedule'] ? date('d-m-Y',strtotime($item['delivery_schedule'])) : '-'}}" id="delivery_dates" class="delivery_dates" disabled>
                                    {{$item['delivery_schedule'] ? date('d-m-Y',strtotime($item['delivery_schedule'])) : '-'}}</td>
									<td>
                                        ...
                                        ?>
                                        <input type="text" name="suppliers[]" value="{{ $supp['supplier']}}" id="suppliers" class="suppliers" disabled>  
                                        <?php
											echo $supp['supplier'];
										?>
										@endif
...
<script>
    function enable(cash)
    {
        const checkbox = $(cash);
        if(checkbox.is(':checked')){
            checkbox.closest('tr').find('.delivery_dates').attr("disabled", false);
            checkbox.closest('tr').find('.suppliers').attr("disabled", false);
            //alert('checked');
        }
        else
        {
            checkbox.closest('tr').find('.delivery_dates').attr("disabled", true);
            checkbox.closest('tr').find('.suppliers').attr("disabled", true);
            //alert('not');
        }
    }
  $(function(){
