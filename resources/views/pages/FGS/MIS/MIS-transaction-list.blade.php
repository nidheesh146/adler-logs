	...
	<div class="container">
		<div class="az-content-body">
            ...
            MIS  Transaction List
			  ...
			  <button style="float: right;font-size: 14px;" onclick="document.location.href='{{url('fgs/fgs-mis-export').'?'.http_build_query(array_merge(request()->all()))}}'" class="badge badge-pill badge-info"><i class="fas fa-file-excel"></i> Report</button>
											...
											<div class="row filter_search" style="margin-left: 0px;">
												<div class="col-sm-10 col-md- col-lg-10 col-xl-10 row">
                                                    ...
                                                    <div class="form-group col-sm-12 col-md-3 col-lg- col-xl-4">
														<label>MIS No :</label>
														<input type="text" value="{{request()->get('cmin_no')}}" name="mis_no" id="mis_no" class="form-control" placeholder="CMIN NO">
													</div><!-- form-group -->
					...
					<div class="table-responsive">
						<table class="table table-bordered mg-b-0" >
                            ...
                            <td>{{$sl++}}</td>
                                ...
                                <td>{{date('d-m-Y',strtotime($item->mis))}}</td>
                            </tr>
                            @endforeach
							</tbody>
						</table>
...
<script>
  $(function(){
	...
	$('.search-btn').on( "click", function(e)  {
		...
		var mis_no = $('#mis_no').val();
        var item_code = $('#item_code').val();
		var from = $('#from').val();
		if(!mis_no  & !item_code & !from)
		{
			e.preventDefault();
		}
	});
