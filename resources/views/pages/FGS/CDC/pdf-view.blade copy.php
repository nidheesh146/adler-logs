...
<body>
    ...
    <div class="row" style="margin-left: 10px; margin-right: 10px;">
        <table style="width: 100%;" id="table">
                ...
                <td colspan="7">
                    <table id="table1" style="width: 100%;">
                        <tr>
                            ...
                            <td style="width: 15%;">{{$cdc->doc_no}}</td>
                        </tr>
                        <tr>

                            <td colspan="3" rowspan="2">{{$cdc->shipping_address}}</td>
                            <td style="font-weight: bold;">Doc Date :</td>
                            <td>{{date('d-m-Y',strtotime($cdc->doc_date))}}</td>
                        </tr>
                        <tr>

                            <td colspan="2">CDC No.: {{$cdc->cdc_number}}<br />
                                CDC. Date: {{date('d-m-Y',strtotime($cdc->cdc_date))}}
                            </td>
            ...
            <tr>
                <td colspan="7">
                    <table id="table2" style="width: 100%;page-break-inside: auto;
">
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>{{$data->batch_no}}</td>
                            <td style="text-align:center;">{{$totalQuantity[]=$data->quantity}}</td>
                            <td>{{date('d-m-Y',strtotime($data->manufacturing_date))}}</td>
                            <td>@if($data->expiry_date=='0000-00-00' || $data->expiry_date==NULL ) N.A @else {{date('d-m-Y',strtotime($data->expiry_date))}} @endif</td>
                        </tr>
                        @endforeach
                        <tr>
                            <th></th>
    ...
    <script type="text/php">
        if (isset($pdf)) {
            $xPage = 540; // X-axis for "Page", positioned on the right side
            ...
            if (var_dump($pageNumber) != 0) {
                $xDoc = 540;  // X-axis for "Doc", positioned on the left side
