...
<div class="az-content az-content-dashboard">
  <br> 
            ...
            <form method="POST" id="commentForm" autocomplete="off" >
                ...
                <div class="row">
                    <div class="form-group col-sm-12 col-md-4 col-lg-4 col-xl-4" data-select2-id="7">
                            ...
                            @if(!empty($edit['pi']))
                            <input type="hidden" name="pi_number" value="{{$edit['pi']->pi_number}}">

                            @endif
                            <select class="form-control pi_number" name="pi_number" @if(!empty($edit['pi'])) disabled @endif>
                                <!-- <option value="" ></option> -->
                                @if(!empty($edit['pi']))
                                    <option value="{{$edit['pi']->pi_number}}" selected>{{$edit['pi']->pi_number}}</option>
                                @endif
                            </select>
                    </div>
                    ...
                    <div class="form-group col-sm-12 col-md-4 col-lg-4 col-xl-4">
                            ...
                            <input type="text" class="form-control datepicker" value="{{date("d-m-Y")}}" name="cpi_date" placeholder="pi date" id="cpi_date">
                    @if(!empty($edit['pi']))
                    <input type="hidden" name="stock_location" value="{{$edit['pi']->stock_location}}">
                    @endif
                </div>
                <div class="form-group col-sm-12 col-md-4 col-lg-4 col-xl-4">
                    <label>Created by: *</label>
                        ...
                        @foreach ($data['users'] as $user)
                            <option value="{{$user->user_id}}"   @if(!empty($edit['pi']) && $edit['pi']->created_by == $user->user_id) selected  @endif   >{{$user->f_name}} {{$user->l_name}}</option>
                            @endforeach                                          
                        </select>
                </div>
    ...
    $('.pi_number').select2({
          ...
          allowClear: true,
          ajax: {
          url: "{{ url('fgs/CPI/find-pi-number-for-cpi') }}",
        ...
        if(res){
          $.get("{{ url('fgs/CPI/find-pi-info') }}?id="+res.id,function(data){
