...
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/css/bootstrap.min.css">
    ...
    <title>COEF_{{$coef['firm_name']}}_{{$coef['coef_number']}}</title
</head>
<body>
    ...
    <div class="row2" style="height:70px;">
        <div class="col21">
            <table>
                <tr>
                    <td>Zone</td>
                    <td>: {{$coef['zone_name']}} </td>
                   
                </tr>
                <tr>
                    
                    <td >GST Details</td>
                    <td>: {{$coef['gst_number']}}</td>
                   
                </tr>
                <tr>
                    <td >D.L. Details</td>
                    <td>: {{$coef['dl_number1']}}, {{$coef['dl_number2']}}, {{$coef['dl_number3']}} </td>
                </tr>
            </table>
        </div>
        <div class="col22">
            <table style="float:left;">
                <tr>
                    <td>Order No.</td>
                    <td>: {{$coef['order_number']}} </td>
                </tr>
                <tr>
                    <td> Order Date</td>
                    <td>: {{date('d-m-Y', strtotime($coef['order_date']))}}</td>
                </tr>
                <tr>
                    <td>Order Fulfil</td>
                    <td>: {{$coef['order_fulfil_type']}} </td>
                </tr>
            </table>
        </div>
        <div class="col23">
        ...
        <div class="col24">
            <table style="float:left;">
                <tr>
                    <td>Doc  No</td>
                    <td>: {{$coef['coef_number']}}</td>
                </tr>
                <tr>
                    <td> Doc  Date</td>
                    <td>: {{date('d-m-Y', strtotime($coef['coef_date']))}}</td>
                </tr>
                <tr>
                    <td>Due Date</td>
        ...
        <table  border ="1">
            ...
            <tr>
               ...
               {{--<td>{{number_format((float)($item['rate']* $item['quantity']), 2, '.', '') }}</td> --}}
                ...
                $cgstsum= $cgstsum+($discount_value*$item['cgst'])/100;
                $sgstsum= $sgstsum+($discount_value*$item['sgst'])/100;
                $igstsum= $igstsum+($discount_value*$item['igst'])/100;
    ...
    <div class="row4" style="border-bottom:solid 1px black;height:170px;">
        <div class="col41">
            <div class="valuewords">
                ...
                <span class="value_in_words"><?php echo( $fn->getIndianCurrencyInt(number_format((int)($total-$total_discount+$total_igst+$total_sgst+$total_sgst), 2, '.', ''))) ?> Only </span>
            </div><br/>
        ...
        <div class="col43">
            ...
            <table style="border-bottom:solid 1px black;width:100%;border-top:solid 1px black;width:100%;">
                <tr>
                    <th style="width:130px;color:#1434A4;">GRAND TOTAL</th>
                    <?php
                     $grand = 0;
                     $grandt = 0;
