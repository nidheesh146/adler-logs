  ...
  <div class="card bd-0">
    <div class="card-header bg-gray-400 bd-b-0-f pd-b-0">
        <nav class="nav nav-tabs">
          <a class="nav-link @if(in_array($Action,['BackorderReport.get_data'])){{'active'}} @endif"  href="{{url('fgs/back-ordr-report')}}">All</a>
          <a class="nav-link @if(in_array($Action,['OEF.pendingOEF'])){{'active'}} @endif"  href="{{url('fgs/OEF/pending-report')}}">OEF</a>
          <a class="nav-link @if(in_array($Action,['GRS.pendingGRS'])){{'active'}} @endif"  href="{{url('fgs/GRS/pending-report')}}">GRS </a>
